...
#import "GCAudioLyricsTableView.h"
#import "GCAudioPlayer.h"

@interface GCAudioLyricsTableViewCell : UITableViewCell
...
@implementation GCAudioLyricsTableViewCell
...
@interface GCAudioLyricsTableView ()<UITableViewDelegate, UITableViewDataSource, UIScrollViewDelegate>
...
@implementation GCAudioLyricsTableView {
...
- (void)dealloc{
    ...
    [[GCAudioPlayer sharedPlayer] removeObserver:self forKeyPath:DFPlayerLyricsStateKey];
    [[GCAudioPlayer sharedPlayer] removeObserver:self forKeyPath:DFPlayerLyricsCurrentTimeKey];
    [[GCAudioPlayer sharedPlayer] removeObserver:self forKeyPath:DFPlayerLyricsCurrentAudioInfoModelKey];
}
...
- (instancetype)init{
    ...
    if (self) {
        ...
        [[GCAudioPlayer sharedPlayer] addObserver:self forKeyPath:DFPlayerLyricsCurrentAudioInfoModelKey options:options context:nil];
        [[GCAudioPlayer sharedPlayer] addObserver:self forKeyPath:DFPlayerLyricsCurrentTimeKey options:options context:nil];
        [[GCAudioPlayer sharedPlayer] addObserver:self forKeyPath:DFPlayerLyricsStateKey options:options context:nil];
    }
    return self;
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context{
    if (object == [GCAudioPlayer sharedPlayer]) {
        if ([keyPath isEqualToString:DFPlayerLyricsCurrentAudioInfoModelKey]){
            ...
            if ([GCAudioPlayer sharedPlayer].playState == ICAudioPlayerStatePlaying) {
...
- (void)df_updateLyricsAnimated:(BOOL)animated{
    ...
    //获取当前行
    CGFloat currentTimeFloat = [GCAudioPlayer sharedPlayer].currentTime;
    ...
    //返回当前行歌词
    if (self.lyricsDelegate && [self.lyricsDelegate respondsToSelector:@selector(gc_lyricsTableView:onPlayingLyrics:)]) {
        NSString *lyrics = self.lyricsArray[_currentIndex];
        if ([lyrics isEqualToString:DFPlayerLyricsConstMark]) {
            lyrics = @"";
        }
        dispatch_async(dispatch_get_main_queue(), ^{
            [self.lyricsDelegate gc_lyricsTableView:self onPlayingLyrics:lyrics];
        });
    }
...
- (void)updateLyricsAnimated:(BOOL)animated{
    ...
    if (self.currentIndexArray.count > 1) {
        for (int i = 0; i < self.currentIndexArray.count-1; i++) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                GCAudioLyricsTableViewCell *cell = (GCAudioLyricsTableViewCell *)[self cellForRowAtIndexPath:indexPath];
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        //当前行
        GCAudioLyricsTableViewCell *cell = (GCAudioLyricsTableViewCell *)[self cellForRowAtIndexPath:self->_currentIndexPath];
...
- (void)setOtherLineLyricsTextStatus:(GCAudioLyricsTableViewCell *)cell indexPath:(NSIndexPath *)indexPath{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    GCAudioLyricsTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (!cell) {
        cell = [[GCAudioLyricsTableViewCell alloc] initWithStyle:(UITableViewCellStyleDefault) reuseIdentifier:identifier];
