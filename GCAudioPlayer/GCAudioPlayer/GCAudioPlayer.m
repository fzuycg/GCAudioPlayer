...
- (void)playAudioWithModel:(GCAudioPlayModel *)model PlayStatue:(BOOL)isPlay{
    _currentPlayModel = model;
    [self.player pause];
    if (!isPlay) {
        //player如果已经被移除，重新创建
...
- (void)prepareToPlayWithPlayitem:(NSURL *)audioURL {
    _isPreparedToPlay = YES;
    [self initializePlayerWithPlayitem:audioURL];
...
- (void)initializePlayerWithPlayitem:(NSURL *)audioURL {
    _asset = [AVURLAsset assetWithURL:audioURL];
    _playerItem = [AVPlayerItem playerItemWithAsset:_asset];
...
- (void)itemObserving {
    [_playerItemKVO safelyRemoveAllObservers];
    _playerItemKVO = [[GCAudioPlayKVOManager alloc] initWithTarget:_playerItem];
    ...
    _timeObserver = [self.player addPeriodicTimeObserverForInterval:interval queue:dispatch_get_main_queue() usingBlock:^(CMTime time) {
        ...
        if (loadedRanges.count > 0 && currentItem.duration.timescale != 0) {
            NSTimeInterval currentTime = currentItem.currentTime.value / currentItem.currentTime.timescale;
            NSTimeInterval totalTime = currentItem.duration.value / currentItem.duration.timescale;
